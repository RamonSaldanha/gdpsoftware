	...
	computed: {
		modelo() {
			return this.modelos.find(
				(modelo) => modelo.nome === this.$route.params.modelo
			);
	...
	methods: {
		...
		async createOpenAIPrompt(promptContent) {
			...
			if (tokensUsed > MAX_TOKENS) {
				this.displayAlert(
					"Seu texto excede o limite de tokens permitidos, tente ser mais suscinto na narração do caso concreto",
					"error"
				);
      ...
      const promptPedidos = `Solicito sua ajuda para redigir o tópico: "Dos pedidos". Este tópico deve conter os pedidos da petição inicial cumprindo a exigência do art. 319. Lembre-se que não precisa redigir todo fechamento da petição (data, oab e nome), apenas os pedidos.`
			...
			try {
				...
				const responseFundamentos = await this.createOpenAIPrompt(
					promptFundamentos
				);
		...
		async handleInputs() {
			...
			try {
				content = fs.readFileSync(
					vm.modelsFolder + platformFileWay() + "peticao-inicial-ia.docx",
					"binary"
				);
			} catch (err) {
				document.fire({
					type: "error",
					title:
						"Não foi possível gerar um documento, selecione um modelo antes!",
				});
				return;
			}
			...
			const fileName = `${
				vm.documentsFolder
			}\\(${today.getDate()}-${month}-${today.getFullYear()} ${today.getHours()}hrs ${today.getMinutes()}min) peticao-inicial.docx`;
